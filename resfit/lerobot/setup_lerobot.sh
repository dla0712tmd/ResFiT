...
# Install git-lfs (required by lerobot repo)
conda install -n residual -c conda-forge git-lfs -y
git lfs install

# Git clone lerobot into deps directory
[ -d "$DEPS_DIR/lerobot" ] || git clone https://github.com/huggingface/lerobot.git "$DEPS_DIR/lerobot"
git -C "$DEPS_DIR/lerobot" reset --hard
